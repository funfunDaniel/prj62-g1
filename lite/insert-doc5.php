<?php
	...
	$v_file_dire = "";
	$v_lg = $_POST["language"];
	$v_col_type = "Template วิทยานิพนธ์";
	$v_date = date("Y-m-d");
    ...
    if ($_FILES["file"]["error"] > 0){ 
        ...
        if (file_exists("upload/doc-for/" . $_FILES["file"]["name"])){ 
            header('Location:add-data-form.php?addType=df5&ext=1');
        }else{
    ...
    $sql = "INSERT INTO document_Forms5 (title,issued_date,description,publisher,subject,type,language,file_directory)
    VALUES('" . $v_title ."','" .$v_date . "','" . $v_des . "','" . $v_publisher . "','" .$v_subject . "','" . $v_type . "','" . $v_lg . "','" . $v_file_dire . "')";
        
    if (mysqli_multi_query($conn, $sql)) {
       	$sql = "SELECT * FROM document_Forms5 ORDER BY df5_id DESC LIMIT 1";
        ...
        if (mysqli_multi_query($conn, $sql)) {
            header('Location:add-data-form.php?addType=df5&alert=1');
        }else{
