...
<div id="page">
    ...
    <?php   
		$sql = "SELECT * FROM graduation_projects";
	...
	<div id="container">
	<form name="searchForm" method="POST" action="" autocomplete="on" enctype="multipart/form-data" >  
	<?php
    ...
    if(isset($_POST['submitBtn'])){
      $sql = "SELECT * FROM graduation_projects WHERE owner LIKE '%" . $_POST['owner'] . "%'";
    }
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) != 0) {
      while($row = mysqli_fetch_array($result)) {
        $output .= " <br/>	
				...
				<p style=\"font-size:16px; text-align: center; vertical-align:middle; \"> รับรางวัลเมื่อวันที่ : " . $row['issued_date'] . "</p>
				<p style=\"font-size:18px; text-align: center; vertical-align:middle; \"> หัวข้อโครงงาน  " . $row['title'] . " ในรายวิชา" . $row['subject'] . "</p>
				<p style=\"font-size:18px; text-align: center; vertical-align:middle; \"> โดยมีอาจารย์ที่ปรึกษาคือ " . $row['advisor'] . "</p> <br/><br/><br/>
				<p style=\"font-size:30px; text-align: center; vertical-align:middle; \">โครงงานประเภท " . $row['type'] . "</p>			
	...
	?>	
		...
		<input type="button" class="btn-save" value="Back" style="float: left"  onclick="window.location='prof-grad-pro-form.php'"  />
