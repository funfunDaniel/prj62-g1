<?php
    ...
    if ($_FILES["file"]["error"] > 0){ 
        ...
        if (file_exists("upload/doc-for/" . $_FILES["file"]["name"])){ 
            header('Location:add-data-form.php?addType=df&ext=1');
        }else{
    ...
    $sql = "INSERT INTO document_Forms (title,issued_date,description,publisher,subject,type,language,file_directory)
    VALUES('" . $v_title ."','" .$v_date . "','" . $v_des . "','" . $v_publisher . "','" .$v_subject . "','" . $v_type . "','" . $v_lg . "','" . $v_file_dire . "')";
        
    if (mysqli_multi_query($conn, $sql)) {
       	$sql = "SELECT * FROM document_Forms ORDER BY df_id DESC LIMIT 1";
        ...
        if (mysqli_multi_query($conn, $sql)) {
            header('Location:add-data-form.php?addType=df&alert=1');
        }else{
